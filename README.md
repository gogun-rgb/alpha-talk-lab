...
- 두 종목의 추세·모멘텀·안정성·상대 강도를 기반으로 한 기술적 매력도 비교
- Markdown 리서치 노트 복사 및 다운로드
- 로딩, 오류, 뉴스 없음, OpenAI 미설정 상태 처리

기술적 매력도는 매수 추천 점수가 아닙니다. 최근 가격 데이터를 여러 규칙으로 요약하여 두 종목의 상대적인 기술적 상태를 비교합니다. 안정성 점수는 변동성, 최대 낙폭, 하방편차, 최근 낙폭, 수익률 대비 변동성을 25점으로 요약하며, 높은 점수가 미래 손실이 없음을 뜻하지는 않습니다. RSI 14는 Wilder 지수 평활 방식으로 계산하고, 하방편차는 `sqrt(mean(min(return, 0)^2)) * sqrt(252)` 방식으로 계산합니다. 기술적 비교의 신뢰도는 미래 수익 확률이 아니라 데이터 충분도, 총점 차이, 세부 영역 방향 일치도, 점수 집중도를 요약한 값입니다.
...
- 뉴스 가용성 검증
- 요청한 두 종목 외의 티커 검증
- 추천·목표주가·수익 보장·미래 상승 확률 표현 검증
- 백엔드 계산값과 충돌하는 퍼센트, 달러 가격, 거래일 수, 상관계수, 기술적 점수, RSI 수치 검증
- 뉴스 기반 가설의 제공 뉴스 ID 참조 여부 검증
- 실패 시 한 번 재시도 후 규칙 기반 리서치 노트로 fallback

## Python이 담당하는 계산

누적 수익률, 일별 수익률, 연환산 변동성, 최대 낙폭, 상승 거래일 비율, 상관계수, 정규화 가격은 모두 `backend/app/services/metrics.py`에서 계산합니다. 기술적 매력도 점수는 `backend/app/services/technical_analysis.py`에서 추세, 모멘텀, 안정성, 상대 강도를 규칙 기반으로 계산합니다.
...
- 기술적 신뢰도는 예측 정확도나 상승 확률이 아닙니다.
- AI 의미 검증은 보수적인 규칙 기반 검사이며 모든 자연어 환각을 탐지하지 못합니다.
